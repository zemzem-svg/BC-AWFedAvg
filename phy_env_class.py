...
from phy.scenario import waterfilling as wf
...
param_under_test = np.around(np.arange(0.3, 0.6, 0.1), 2).tolist()
...
class Phy:
    def __init__(self,
        ...
        # Track URLLC successes for metrics
        self._urllc_success_counter = 0  # track successful URLLC transmissions
    ...
    def env_get_state(self, only_shape=False):
        ...
        if not self.episode_done:
            ...
            for i, pkt in enumerate(new_pkt):  # i is the URLL user
                ...
                self.urllc_latency_left[i] = self.tolerable_latency[i] - (self.step_number + 1 - self.urllc_queue[i][0]
        ...
        normalized_state = self.normalize_state(np.concatenate((self.urllc_state, self.embb_state)))
        return normalized_state

    def normalize_state(self, state):
        """Normalize state components for better learning stability"""
        normalized = state.copy()
        # Normalize queue length - divide by maximum possible queue length
        # (which could be the tolerable latency or a larger constant)
        if normalized[0] > 0:  # Only normalize if there's a queue
            normalized[0] = normalized[0] / (self.step_max + 1)
        
        # Normalize latency left - scale to [-1, 1] range
        if len(self.urllc_queue[0]) > 0:  # Only if there are packets in queue
            normalized[1] = normalized[1] / self.tolerable_latency[0]
            
        # eMBB state values are already normalized, as they represent codeword classes
        return normalized
    ...
    def env_step(self, action):
        ...
        # Priority mechanism for URLLC packets close to deadline
        if len(self.urllc_queue[0]) > 0 and self.urllc_latency_left[0] <= 2:
            # If there's a URLLC packet close to its deadline, find the best frequency to puncture
            # (one that minimizes impact on eMBB)
            best_freq = np.argmin([self.cw_fun[self.cw_ref[self.step_number, f]] 
                                  for f in range(self.cluster.subs)])
            pun = best_freq + 1  # +1 because pun=0 means not transmitting
        else:
            # Otherwise use the action from the agent
            pun = self.action_space[action]

        outage = 0  # initialization: no codeword in outage because of my action
        # Track if we successfully transmitted a URLLC packet
        urllc_success = False
        
        # ACTION: Puncturing mode
        if pun:  # If pun == 0 the action means not transmit the URLLC pkt
            # Check if there's actually a packet to transmit
        ...
        if self.rl_ver == 'bernoulli':
            self.reward[self.step_number] = outage - latency_fun - self.q_norm * self.q_len[0]
    ...
    def env_render(self):
        ...
        if self.step_number <= self.step_max:
            with open(_render_file, "a") as rf:
                ...
                data_str += ','.join(f'{el:.4f}' for el in self.state_previous) + '\n'
                data_str += ','.join(f'{el:.4f}' for el in np.concatenate((self.urllc_state, self.embb_state)))  # STATE
                if self.rl_ver <= 4:
    ...
    @property
    def urllc_success_counter(self):
        return self._urllc_success_counter
